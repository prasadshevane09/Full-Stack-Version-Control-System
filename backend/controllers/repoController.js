...
async function fetchRepositoryById (req, res) {
    ...
    console.error("Error during fetching repo by id : ", err.message);
    res.status(500).send("Server error!");
  }
};
...
async function fetchRepositoryByName (req, res) {
    ...
    console.error("Error during fetching repo by name : ", err.message);
    res.status(500).send("Server error!");
  }
};
...
async function fetchRepositoriesForCurrentUser (req, res) {
    ...
    res.json({message:"Repositories found", repositories});
        ...
        console.error("Error during fetching user repoes: ", err.message);
        res.status(500).send("Server error!");
    }
};
...
async function updateRepositoryById (req, res) {
    ...
    try{
        ...
        if(!repository){
        return res.status(404).json({error: "Repositorynot found!"});
        }
...
async function toggleVisibilityById (req, res) {
    ...
    try {
       ...
       repository.visibility = !repository.visibility;
        ...
        console.error("Error during toggling repo: ", err.message);
        res.status(500).send("Server error!");
    }
};
